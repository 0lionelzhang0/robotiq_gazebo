...
RobotiqHandPlugin::RobotiqHandPlugin()
{
	...
	{
		this->posePID[i].Init(2.0, 0, 0.1, 0.0, 0.0, 60.0, -60.0);
...
void RobotiqHandPlugin::UpdateStates()
{
	...
	if (curTime > this->lastControllerUpdateTime)
	{
		...
		else
		{
			...
			if (static_cast<int>(this->handleCommand.rMOD) != this->graspingMode && !this->ICS_on)
			{
	...
	for (int i = 0; i < this->NumJoints; ++i)
	{
		...
		else if (i >= 2 && i <= 4) // joint 1 of all fingers
		{
			...
			else
			{
				...
				// printf("Handle A: %d\n", this->handleCommand.rPRA);
				// printf("Handle B: %d\n", this->handleCommand.rPRB);
